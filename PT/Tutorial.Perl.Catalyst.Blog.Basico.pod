...
=head1 TUTORIAL PERL CATALYST - COMO CRIAR UM BLOG BASICO
...
agora acesse sua página inicial em http://localhost:3005/ e veja o cabeçalho da página ali... observe que suas páginas static podem ficar dentro do diretório /static/images... 
...
vamos criar então duas tabelas em qualquer banco de dados... vou demonstrar utilizando postgres ( a vantagem ao utilizar postgres com catalyst é que ele já detecta os relacionamentos):
...
e agora vamos criar nossos models para nossa app.. vamos utilizar o dbic dump ( mais info DBIx::Class::Schema::Loader )
    ...
    cd Blog-Tutorial
    script/blog_tutorial_create.pl model DB DBIC::Schema Blog::Tutorial::DB create=static dbi:Pg:dbname=blog_tutorial webdev passworrrddzz
...
bom, agora já especificamos os relacionamentos de nossos models... quando vc utiliza postgres ele detecta os relacionamentos automaticamente.. exceto os many to many porque na verdade não many-to-many não é relacionamento.
...
remova o método index e adicione dois novos métodos: 'login' e 'logout'... ficando com um arquivo assim:
...
e insira o método 'auto' que provavelmente não estará lá por padrão:
